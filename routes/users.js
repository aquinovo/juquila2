...
evangelio1=function(req, res) {
	console.log(req.body); 
	if(req.body.usuario=="santuario" && req.body.contrasena=="virgenjuquila"){	
		fs.writeFile("./evangelio.txt", req.body.link, function (err) {
		    if (err) {
		    	return console.log(err);
		    }
		    res.send('true');
...
evangelio2=function(req, res) {
	fs.readFile('./evangelio.txt', 'utf-8', (err, data) => {
	  if(err) {
	    console.log('error: ', err);
	    res.send("false");
	  } else {
...
router.post('/evangelio',evangelio1);
router.get('/evangelio',evangelio2);
